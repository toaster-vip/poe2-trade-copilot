(() => {
  ...
  const VERSION = "remote-bootstrap-1.5";
  ...
  // Critical modules affect search/result behavior and must stay minimal.
  ...
  // Optional UI modules are isolated. A UI failure must never break the trade page.
  ...
  async function boot() {
    ...
    try {
      ...
      for (const path of OPTIONAL_MODULES) {
        ...
        else optionalErrors.push({path:result.path,error:result.error});
      }
      ...
      const showStatus=()=>{
        ...
        el.textContent = optionalErrors.length
          ? `Remote ${VERSION} · core ready · optional UI degraded`
          : `Remote ${VERSION} · core ready`;
      };
      showStatus();
    } catch(error) {
      console.error("[PoE2TC Remote Bootstrap] Critical boot failed:",error);
      const box=document.createElement("div");
      box.textContent=`PoE2 Trade Copilot critical startup failed: ${error.message}`;
