...
// @version      1.1.0
// @description  Collects all PoE2 trade results and saves them directly to this project's GitHub repo
...
(() => {
  ...
  function cardElements() {
    ...
    const out = [];
    for (const selector of selectors) for (const el of $$(selector)) {
  ...
  function num(text, re) { const m = text.match(re); return m ? Number(String(m[1]).replace(/,/g, "")) : null; }
  ...
  function parseCard(card) {
    ...
    return {
      ...
      itemLevel:num(text,/Item Level:\s*([0-9]+)/i), quality:num(text,/Quality:\s*\+?([0-9]+)%/i),
  ...
  function findScrollContainer() {
    const result=$(".resultset")||$(".search-results"); if(!result) return null;
    let el=result; while(el&&el!==document.body){ const s=getComputedStyle(el); if((s.overflowY==="auto"||s.overflowY==="scroll")&&el.scrollHeight>el.clientHeight+20) return el; el=el.parentElement; }
    return null;
  }
  async function collectAllResults() {
    const map=new Map(), container=findScrollContainer();
    ...
    const original=getPos(); setPos(0); await sleep(300); let last=-1,stagnant=0;
    for(let rounds=0;rounds<150;rounds++){
      collectCurrent(map); status(`Collecting… ${map.size} captured`);
      const max=getMax(), pos=getPos(); stagnant=map.size===last?stagnant+1:0; last=map.size;
      if(pos>=max-5){ await sleep(600); collectCurrent(map); const max2=getMax(); if(stagnant>=3&&max2<=max+5) break; }
      setPos(Math.min(getMax(),pos+stepSize())); await sleep(350);
    }
    setPos(original); await sleep(100); return [...map.values()];
  }
  ...
  function githubRequest({method="GET",url,token,body}){
    return new Promise((resolve,reject)=>{
      GM.xmlHttpRequest({method,url,headers:{Accept:"application/vnd.github+json",Authorization:`Bearer ${token}`,"X-GitHub-Api-Version":"2022-11-28",...(body?{"Content-Type":"application/json"}:{})},data:body?JSON.stringify(body):undefined,timeout:30000,
        onload:r=>{let json=null;try{json=r.responseText?JSON.parse(r.responseText):null}catch{} if(r.status>=200&&r.status<300)resolve({status:r.status,json});else reject(new Error(`GitHub HTTP ${r.status}: ${json?.message||r.responseText||"request failed"}`));},
        onerror:()=>reject(new Error("GitHub network request failed")),ontimeout:()=>reject(new Error("GitHub request timed out"))});
    });
  }
  ...
  async function existingSha(token){ const url=`https://api.github.com/repos/${OWNER}/${REPO}/contents/${LATEST_PATH}?ref=${BRANCH}`; try{return (await githubRequest({url,token})).json?.sha||null}catch(e){if(/HTTP 404/.test(e.message))return null;throw e;} }
  async function uploadPacket(packet){ const token=await getToken(); if(!token) throw new Error("TOKEN_NOT_SET"); const sha=await existingSha(token); const url=`https://api.github.com/repos/${OWNER}/${REPO}/contents/${LATEST_PATH}`; return githubRequest({method:"PUT",url,token,body:{message:`Update latest trade results (${packet.listings.length} listings)`,content:base64Utf8(JSON.stringify(packet,null,2)+"\n"),branch:BRANCH,...(sha?{sha}:{})}}); }

  async function saveAllResults(){
    try{
      if(!(await getToken())){status("Set GitHub token once before saving results.");return;}
      status("Collecting all trade results…"); const listings=await collectAllResults();
      const packet={protocol:"poe2-trade-copilot/results-v5",version:`0.5.1+github-save-${VERSION}`,capturedAt:new Date().toISOString(),sourceUrl:location.href,visibleResults:listings.length,listings};
      status(`Uploading ${listings.length} result(s) to GitHub…`); const result=await uploadPacket(packet); const sha=result.json?.commit?.sha||"";
      status(`Saved ${listings.length} result(s) to GitHub${sha?` · ${sha.slice(0,7)}`:""}.`);
    }catch(error){console.error("[PoE2TC GitHub Save]",error);status(error.message==="TOKEN_NOT_SET"?"GitHub token not set.":`GitHub save failed: ${error.message}`);}
  }

  function install(){
    const resultsButton=$("#ptc-results");
    if(!resultsButton){setTimeout(install,300);return;}
    const grid=resultsButton.parentElement;
    ...
    if(!$("#ptc-token")){
      ...
      tokenButton.id="ptc-token"; tokenButton.type="button"; tokenButton.textContent="SET GITHUB TOKEN";
      tokenButton.setAttribute("style",resultsButton.getAttribute("style")||"");
      ...
      tokenButton.onclick=async()=>{
        const entered=window.prompt("Paste a fine-grained GitHub token for toaster-vip/poe2-trade-copilot with Contents: Read and write. It stays only in Userscripts storage on this device.","");
        if(entered==null)return; const token=String(entered).trim(); await setToken(token); status(token?"GitHub token saved locally.":"GitHub token cleared.");
      };
      grid.appendChild(tokenButton);
    }
    console.log(`[PoE2TC GitHub Save] ${VERSION} installed.`);
  }
